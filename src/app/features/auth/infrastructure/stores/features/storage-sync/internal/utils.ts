 ...
 * @param {StorageSyncConfig<State>} config The synchronization configuration object.
...
export function discoverStateKey<State extends object>(
	...
	const discoveryProxy = new Proxy({} as never, {
		get(_, prop) {
			...
			const nestedProxy = (): unknown => new Proxy({}, { get: () => nestedProxy() });
			return nestedProxy();
		},
	});

	try { config.select(discoveryProxy); } catch {}
...
export function normalizeSyncConfigs<State extends object>(
	...
	return configs.map((config) => ({ ...config, discoveredKey: discoverStateKey(config) }));
}
...
export function writeValueToStorage(
	...
	try {
		const valueToStore = stringify ? stringify(dataToSave) : JSON.stringify(dataToSave);
